...
class Cli:
    ...
    def run(self, params: Union[str, dict]) -> bool:
        ...
        for website in params["websites"]:
            if website == "东方财富网":
                ...
                continue
            scrape_tasks.append((website, scraper_class, params["time_range"]))

        if len(scrape_tasks) == 0:
            return False

        self._run_scrape_tasks(scrape_tasks)

        self._merge_news_files()
    def _run_scrape_tasks(self, scrape_tasks):
        ...
        with ThreadPoolExecutor(max_workers=params["max_workers"]) as executor:
            ...
            future_to_website = {
                executor.submit(
                    self._scrape_single_website, website, scraper_class, time_range
                ): website
                for website, scraper_class, time_range in scrape_tasks
            }
            ...
            for future in as_completed(future_to_website):
                ...
                try:
                    ...
                    if result:
                        ...
                        print(f"✓ {website} 抓取成功，文件名: {result}")
                    else:
                        failed_scrapes.append(website)
                        print(f"✗ {website} 抓取失败")
    ...
    def _scrape_single_website(
        self, website: str, scraper_class, time_range: int
    ) -> str:
        """
        抓取单个网站的新闻
        :param website: 网站名称
